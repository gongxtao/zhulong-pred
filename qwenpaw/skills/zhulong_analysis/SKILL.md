...
## ⚠️ 数据现状（2026-08-30 起，最高优先级）——两表内容对调

生产库两表内容于 2026-08-29 晚被管线交叉写入（数据侧已冻结，此为稳定现状，用户裁决）：

- **`pred_dynamic` 表实为「初始静态模型」批回测**（68,040 行 = 945 起点×72 整，与 8/28 快照逐值一致）
- **`pred_static` 表实为「持续学习模型」回放**（68,043 行，精度更优，如 2018-06 AEP MAPE ≈2.94）

**读表规则（覆盖下文表字典的表名语义）**：

- 用户问「持续学习 / 动态 / P50」→ 查 **`pred_static` 表**
- 用户问「静态 / 对照 / 基线模型」→ 查 **`pred_dynamic` 表**
- 口径行照实写表名并注明内容（如「pred_static 表（内容=持续学习输出）」），结论按内容语义给。

下方表字典与食谱代码中的表名沿用原名——执行查询时按上表换读。
参考对账值同理反读：2016-03 AEP 对齐 744 行 → `pred_static` 表（=持续学习）MAPE ≈2.75、
`pred_dynamic` 表（=静态）≈3.00。（若未来两表归位：删除本节即恢复原语义。）
