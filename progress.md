...
## 2026-08-30 凌晨 II · feat-026 P50 线 25–48h 缝纫（用户提议）+ 两轮 verify 抓回归

### 动机（用户连环追问）

用户发现重演视图"很多时间点 P50 没静态线近"。逐点胜率探针（公平段 h1-24 同起点对齐）：
持续学习内容在 53~63% 小时更近、平均 MAPE 低 0.2~0.3pp——逐点互有胜负是两条高相关日前
预测的正常形态；但更大结构性原因：P50 线单起点 24h 契约，h25–48 回退相似日基线，而静态线
缝纫式全程真模型值——右半段是"基线 vs 模型"的不公平对比。用户裁决：把持续学习次日起点
真值也缝进来（历史起点全都存在）。

### 实现

- `store.ts` 新增 `predSewAt(zone, ts)`：展示轨缝纫查询（覆盖该小时的最近起点 mh∈[1,24]），
  dbgHook 暴露为 ZL_DATA.predSew（断言钩子）
- `forecast.ts` forecastAt 注入块后加缝纫段：**只缝 h>24**；`SEW` 开关让 buildCal 用无缝纫
  形态算残差（缓存键加 `|nosew` 防两种形态互撞）
- `engine.ts` 口径三处：Ⓘ预测条「25–48h：逐日缝纫…无覆盖段回退相似日基线；分位带沿用
  残差标定（偏保守）」、Ⓘ回测条「25–48h 为逐日缝纫延伸，不入审计（契约=24h）」、图例
  「中位路径 · 0–24h 模型 / 25–48h 逐日拼接」
- `verify.mjs` 新断言：2016-06 重演窗 h25–48 段 P50 与 ZL_DATA.predSew 逐点对账

### 两轮 verify 抓回归（本次纪律的红利）

1. 第一版把非注入小时全缝了（含 h≤24 尾段）→ 首屏快照 MAPE 3.57→2.89、§2 超时。
   根因：dayTs 锚 05:00Z 与 pred 日起点 03:00/04:00Z 错位 1–2h，审计窗 h23–24 本就回退
   基线（已录基线含它）。修复=缝纫加 `h>24` 守卫，审计路径逐字节还原。
2. 第二版 MAPE 3.39 ✓ 但 cov 88.8→88.5/54.2→53.4。根因：buildCal ±2h 池化把 h25 新残差
   （缝纫后=模型残差）渗进 h23/24 分位标定。修复=SEW 开关：标定用无缝纫形态。
