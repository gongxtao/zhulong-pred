...
## 2026-08-29 深夜 III · feat-025 主图双线图例名字互换（用户裁决）

- **指令**：「将持续学习 P50 和静态预测的图例更换一下」→ 追问确认 = 名字互换即可。
- **改动**（engine.ts 五处，线的颜色/数据/宽度/层级全不动）：
  ①灰细虚线（predStatic 数据）series 名 静态预测→持续学习 P50，线尾标 静态→P50；
  ②橙色虚线（展示轨 fc P50）series 名 持续学习 P50→静态预测，线尾标 P50→静态；
  ③④tooltip 两行对调（▤ 静态预测=橙轨 p50 值；▤ 持续学习 P50=静态轨 stat 值，P10–P90/P25–P75 带行随橙轨）；
  ⑤图例表两行换位（dash 橙行=静态预测·初始模型对照…；thin 灰行=持续学习 P50·中位路径·学习模型），说明随名字走保持行内自洽。
- **验证**：tsc 0 错；浏览器实测 series 映射（颜色/点数/宽度）、endLabel 映射、图例表顺序、悬停 tooltip 两行新文案全部就位；verify 相关断言（系列名 find/legend 文本/tooltip 正则/markArea=0）兼容互换无需改动（套件仍因 §2 数据漂移中断在先，见深夜 II 条）；截图 .shots/web_v12_legendswap_dark.png。
- 注：编辑过程中一次笔误（DoW_RED/width 1.7）当即发现并修正，最终 Read 复核两系列块无残留。
