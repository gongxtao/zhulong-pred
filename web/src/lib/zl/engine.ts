...
function renderCaliber() {
  ...
  const rows = [
    ...
    ['预测', real
      ? '<b>0–24h：生产模型</b>（梯度提升回归，Supabase model_versions 注册；pred_static/pred_dynamic 双轨注入——运营推送值优先、回测值兜底，分位带由 pred_static 残差标定）；<b>25–48h：逐日缝纫</b>——取展示轨「当时生效」的次日日前预测逐小时铺满（与静态对照线同款，无未来信息泄露），无覆盖段回退相似日基线；分位带沿用残差标定（偏保守）。'
    ...
    ['回测', real
      ? '页面内滚动 28 起点 × 24h 复算（模型契约视界）；另嵌入训练管道独立回测 pred_static（579 日起点）双轨互证。25–48h 为逐日缝纫延伸，不入审计（契约=24h）。'
...
function renderLegendTable() {
  $('legendTable').innerHTML = [
    ...
    ['<span class="sw dash"></span>', '持续学习 P50', '中位路径 · 0–24h 模型 / 25–48h 逐日拼接'],
