...
> 最后更新：2026-08-29 14:10（feat-011 完成：历史视窗实时校准 + 主图历史峰值线移除）· 提交截止：8/29 24:00

## 0. 一句话现状

**web/ 正式工程已就绪并通过全部断言（feat-008 平移 + feat-010 秒开 + feat-011 实时校准）**：
`web/src/lib/zl/`（util/const/sim/store/supabase/forecast/engine）+ React 静态骨架 + globals.css。
**boot = 快照秒开（~1s 全量渲染）→ 后台同步近窗/日峰/pred（~12s）→ 徽章「在线 · Supabase」收敛 §5 基线**；
**查看任一历史段时按视窗实时查询生产库校准（SWR per view，溯源标记 liveDays/livePredOrigins，
toast + 真实请求可见，同窗二次访问零重查）**；离线保持快照；默认深色；时光机重演三入口退出（决策条
↩ / 重演 chip ✕ / 胶片磁吸拖到最右段）；主图历史峰值线已按用户裁决移除（距纪录数字保留在悬停/依据弹层）。
断言脚本 `web/scripts/verify.mjs`（playwright-core 有头独立实例，26 项全过）。
dev：`cd web && npm run dev -- --port 3100`。原型冻结为 spec 与兜底（原型侧上帝视角联动细节 web 未同步，收口对齐）。
